...
impl DawnInstance {
    ...
    pub fn new() -> Result<Self, DawnError> {
        ...
        Err(DawnError::NotFullyImplemented(
            "Dawn FFI stubs not yet connected to built library. \
             Build system is in place, but runtime integration is pending.".to_string()
        ))
    }
    ...
    pub fn request_adapter(&self, _power_preference: DawnPowerPreference) -> Result<DawnAdapter, DawnError> {
        Err(DawnError::NotFullyImplemented(
