    ...
    fn validate(&mut self) -> bool {
        ...
        if self.color_load_op == LoadOpChoice::Clear
            && (self.clear_color_r < 0.0
                || self.clear_color_r > 1.0
                || self.clear_color_g < 0.0
                || self.clear_color_g > 1.0
                || self.clear_color_b < 0.0
                || self.clear_color_b > 1.0
                || self.clear_color_a < 0.0
                || self.clear_color_a > 1.0)
        {
            self.validation_error =
                Some("Clear color values must be between 0.0 and 1.0".to_string());
            self.success_message = None;
            return false;
        }
        ...
        if self.enable_depth_stencil
            ...
            && (self.clear_depth < 0.0 || self.clear_depth > 1.0)
        {
            self.validation_error =
                Some("Clear depth value must be between 0.0 and 1.0".to_string());
            self.success_message = None;
            return false;
        }
        ...
        if self.enable_timestamp_writes {
            ...
            if beginning_index.is_err() {
                self.validation_error =
                    Some("Timestamp beginning index must be a valid number".to_string());
                self.success_message = None;
                return false;
            }
            ...
            if end_index.is_err() {
                self.validation_error =
                    Some("Timestamp end index must be a valid number".to_string());
                self.success_message = None;
                return false;
            }
        }
