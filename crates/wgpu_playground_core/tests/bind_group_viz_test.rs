...
fn test_visualizer_with_basic_layout() {
    ...
    let binding_assignments = vec![
        (0, "MVP Matrix Uniform".to_string()),
    ];
...
fn test_binding_type_colors_are_distinct() {
    ...
    let storage_color = viz.get_binding_type_color(&BindingTypeConfig::StorageBuffer { read_only: true });
...
fn test_shader_visibility_combinations() {
    ...
    let configs = vec![
        ShaderStagesConfig { vertex: true, fragment: false, compute: false },
        ShaderStagesConfig { vertex: false, fragment: true, compute: false },
        ShaderStagesConfig { vertex: false, fragment: false, compute: true },
        ShaderStagesConfig { vertex: true, fragment: true, compute: false },
        ShaderStagesConfig { vertex: true, fragment: true, compute: true },
    ];
    ...
    for config in configs {
        ...
        if has_visibility {
            assert!(layout_entry.visibility.vertex || 
