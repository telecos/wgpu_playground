...
fn test_load_obj_cube() {
    ...
    assert!(!model.meshes.is_empty(), "Model should have at least one mesh");
    ...
    assert!(model.vertex_count >= 8, "Cube should have at least 8 vertices");
    
    // Cube should have 36 indices (12 triangles * 3 indices)
    assert_eq!(model.index_count, 36, "Cube should have 36 indices (12 triangles)");
}
...
fn test_create_buffers_from_model() {
    pollster::block_on(async {
        ...
        assert!(result.is_ok(), "Failed to create buffers: {:?}", result.err());
        
        let (vertex_buffer, index_buffer) = result.unwrap();
        
        // Verify buffers were created (non-zero size)
        assert!(vertex_buffer.size() > 0, "Vertex buffer should have non-zero size");
        assert!(index_buffer.size() > 0, "Index buffer should have non-zero size");
    });
}
