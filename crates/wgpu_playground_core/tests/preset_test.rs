...
fn test_get_all_presets() {
    ...
    assert!(presets.len() >= 3, "Expected at least 3 presets, got {}", presets.len());
...
fn test_pbr_material_preset() {
    ...
    assert!(pbr.state.shader_editor.is_some(), "PBR preset should have shader");
    assert!(pbr.state.buffer_panel.is_some(), "PBR preset should have buffer");
    assert!(pbr.state.render_pipeline_panel.is_some(), "PBR preset should have render pipeline");
    
    // Check shader contains PBR-related code
    let shader = pbr.state.shader_editor.as_ref().unwrap();
    assert!(shader.source_code.contains("PBR"), "Shader should mention PBR");
    assert!(shader.source_code.contains("Cook-Torrance") || shader.source_code.contains("BRDF"), 
...
fn test_shadow_mapping_preset() {
    ...
    // Check that it has the required configurations
    assert!(shadow.state.shader_editor.is_some(), "Shadow preset should have shader");
    assert!(shadow.state.texture_panel.is_some(), "Shadow preset should have texture for shadow map");
    assert!(shadow.state.sampler_panel.is_some(), "Shadow preset should have sampler");
    assert!(shadow.state.buffer_panel.is_some(), "Shadow preset should have buffer");
    
    // Check texture is depth texture
    let texture = shadow.state.texture_panel.as_ref().unwrap();
    assert!(texture.format.contains("Depth"), "Shadow map should be a depth texture");
    
    // Check sampler has comparison
    let sampler = shadow.state.sampler_panel.as_ref().unwrap();
    assert!(sampler.compare.is_some(), "Shadow sampler should have comparison enabled");
}
...
fn test_post_processing_preset() {
    ...
    // Check that it has the required configurations
    assert!(post_process.state.shader_editor.is_some(), "Post-processing preset should have shader");
    assert!(post_process.state.texture_panel.is_some(), "Post-processing preset should have texture");
    assert!(post_process.state.sampler_panel.is_some(), "Post-processing preset should have sampler");
    assert!(post_process.state.buffer_panel.is_some(), "Post-processing preset should have buffer");
    
    // Check shader contains post-processing effects
    let shader = post_process.state.shader_editor.as_ref().unwrap();
    assert!(shader.source_code.contains("vignette") || shader.source_code.contains("bloom") || 
...
fn test_preset_categories() {
    ...
    for category in &categories {
        ...
        assert!(count > 0, "Category {:?} should have at least one preset", category);
    }
}
...
fn test_preset_state_serialization() {
    ...
    for preset in presets {
        ...
        let json = serde_json::to_string(&preset.state)
            .expect(&format!("Failed to serialize preset: {}", preset.id));
        
        assert!(!json.is_empty(), "Serialized JSON should not be empty for preset: {}", preset.id);
        
        let _deserialized: wgpu_playground_core::state::PlaygroundState = 
            serde_json::from_str(&json)
            .expect(&format!("Failed to deserialize preset: {}", preset.id));
    }
}
...
fn test_preset_tags() {
    ...
    for preset in presets {
        ...
        assert!(!preset.tags.is_empty(), "Preset {} should have at least one tag", preset.id);
...
fn test_preset_metadata_completeness() {
    ...
    for preset in presets {
        ...
        assert!(preset.description.len() >= 50, 
                "Preset {} has a short description ({})", preset.id, preset.description.len());
    }
}
