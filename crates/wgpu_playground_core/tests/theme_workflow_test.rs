...
fn test_theme_switching_workflow() {
    ...
    state.save_to_file(&state_file).expect("Failed to save state");
    
    // Step 5: Simulate app restart - load state from file
    let loaded_state = PlaygroundState::load_from_file(&state_file)
...
fn test_theme_independence_from_other_state() {
    ...
    // Load and verify
    let loaded = PlaygroundState::load_from_file(&state_file)
